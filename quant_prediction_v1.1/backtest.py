...
strategy_rules = """
回測規則:
1. 進場條件：模型預測機率 p >= 0.5，以當日收盤價 (Close) 買入。
2. 風控邏輯 (T+1 ~ T+2)：
# %%
   - 停損：若最低價 (Min) 低於買價 2%，以最低價出場。
   - 停利：若收盤價 (Close) 高於買價 3%，以收盤價出場。
3. 強制出場：若前兩日未觸發條件，於 T+3 日以收盤價 (Close) 出場。
4. 摩擦成本：考慮 0.3% (0.003) 交易手續費與稅。
"""
...
for d in date:
    ...
    if (p >= 0.5):
        ...
        if deadline_1day < buy*0.98:
            sell_points.append(deadline_1day)
            continue
        if deadline_2day < buy*0.98:
            sell_points.append(deadline_2day)
            continue
        if future_1day > buy*1.06:
            sell_points.append(future_1day)
            continue
        if future_2day > buy*1.06:
